...
void TrackingAction::PreUserTrackingAction(const G4Track* aTrack )
{
  ...
  if (aTrack->GetTrackID() == 1) {
    fXstartAbs = fDetector->GetxstartDet();
    fXendAbs   = fDetector->GetxendDet();
...
void TrackingAction::PostUserTrackingAction(const G4Track* aTrack)
{
  ...
  /*
  if ((aTrack->GetTouchableHandle()->GetVolume() == fDetector->GetDetector()) &&
  		!(charge =! 0.)) {
  	analysisManager->FillNtupleDColumn(0, position.x()/mm);
  	analysisManager->FillNtupleDColumn(1, position.y()/mm);
  	analysisManager->FillNtupleDColumn(2, position.z()/mm);
  	analysisManager->FillNtupleDColumn(3, fEventAction->fEnergyDeposit/keV);
