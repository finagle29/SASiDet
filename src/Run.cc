...
void Run::EndOfRun()
{
  ...
  //
  const G4Material* material = fDetector->GetDetectorMaterial();
  G4double length      = fDetector->GetDetectorThickness();
  ...
  // Leave Histogram 1 unnormalized
...
G4double Run::ComputeMscHighland()
{
  ...
  G4double t = (fDetector->GetDetectorThickness())
              /(fDetector->GetDetectorMaterial()->GetRadlen());
