...
void SteppingAction::UserSteppingAction(const G4Step* aStep)
{
  if (aStep->GetPreStepPoint()->GetTouchableHandle()->GetVolume() 
    != fDetector->GetDetector()) return;
    
  fEventAction->AddEnergy (aStep->GetTotalEnergyDeposit());
  
  if (aStep->GetTotalEnergyDeposit() > 0.) {
	G4ThreeVector pos = aStep->GetPreStepPoint()->GetPosition();
  	fEventAction->SetPosition(pos);
    // I don't think I used this. Similar code found in EventAction.cc
    // --Milan
  	/*
  	...
  	analysisManager->FillNtupleDColumn(2, pos.z()/mm);
  	analysisManager->FillNtupleDColumn(3, aStep->GetTotalEnergyDeposit()/keV);
